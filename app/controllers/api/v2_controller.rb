class Api::V2Controller < ActionController::Base
  ...
  def get_data()
    params.permit(:action, :email, :territory,:timestamp, :command, :signature)
    @result = ""
    url_verified, data = self.verify_sign_url(request.original_url,["email", "territory", "timestamp","command"])
    if url_verified
  ...
  def get_militants_from_territory(params)
    ...
    if vc_query.any?
      ...
      User.militant.where(vote_circle_id: vc_ids).find_each do |u|
        data << {first_name: u.first_name,phone: u.phone,autonomy_name: u.autonomy_name,province_name: u.province_name,island_name: u.island_name,town_name: u.town_name}
      end
      @result = data
    else
