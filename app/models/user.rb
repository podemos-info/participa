class User < ActiveRecord::Base
  ...
  def still_militant?
    result = self.verified? && self.in_circle? && (self.exempt_from_payment? || self.has_min_monthly_collaboration? || self.collaborations.where.not(frequency:0).where(status:[0,2]).exists?)
