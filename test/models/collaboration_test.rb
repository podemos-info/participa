...
class CollaborationTest < ActiveSupport::TestCase
  ...
  test 'should validations on collaborations work' do
    ...
    assert_not c.save
    assert(
      c.errors[:payment_type].include?('no puede estar en blanco')
    )
    assert(
      c.errors[:amount].include?('no puede estar en blanco')
    )
    assert(
      c.errors[:non_user_email].include?('no puede estar en blanco')
    )
    assert(
      c.errors[:non_user_document_vatid].include?('no puede estar en blanco')
    )
    assert(
      c.errors[:non_user_data].include?('no puede estar en blanco')
    )
    assert(
      c.errors[:user].include?(
        'La colaboración debe tener un usuario asociado.'
      )
    )
  ...
  test 'should national and international scopes work' do
    FactoryBot.create(:collaboration, :ccc)
  ...
  test 'should .validates_not_passport work' do
    ...
    assert(
      collaboration.errors[:user].include?(
        'No puedes colaborar si no dispones de DNI o NIE.'
      )
    )
  end
  ...
  test 'should .validates_age_over work' do
    ...
    user.update_attribute(:born_at, Time.now - 10.years)
    @collaboration.user = user
    assert_not @collaboration.valid?
    assert(
      @collaboration.errors[:user].include?(
        'No puedes colaborar si eres menor de edad.'
      )
    )
  end
  ...
  test 'should .validates_ccc work' do
    ...
    assert_not @collaboration.valid?
    assert(
      @collaboration.errors[:ccc_dc].include?(
  ...
  test 'should .validates_iban work' do
    ...
    assert_not @collaboration.valid?
    assert(
      @collaboration.errors[:iban_account].include?(
        'Cuenta corriente inválida. Dígito de control erroneo. Por favor revísala.'
      )
    )
  ...
  test 'should .is_bank_national? work' do
    ...
    @collaboration.update_attributes(
      payment_type: 3,
      iban_account: 'ES4621770993232366222222'
    )
    assert @collaboration.is_bank_national?
    @collaboration.update_attributes(
      payment_type: 3,
      iban_account: 'DE123123123123123123'
    )
    assert_not @collaboration.is_bank_national?
  end
  ...
  test 'should .is_recurrent? work' do
    assert_equal @collaboration.is_recurrent?, false
  end
  ...
  test 'should .payment_processed! work' do
    ...
    assert_nil credit_card.redsys_identifier
    assert_nil credit_card.redsys_expiration
  end
  ...
  test 'should .charge! work' do
    ...
    stub_request(:post, order.redsys_post_url).to_return(
      status: 200,
      body: '<!-- +(RSisReciboOK)+ -->',
      headers: {}
    )
